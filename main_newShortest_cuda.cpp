	...
	{	
		...
		*/
		...
		for (int i=0; i < numberOfVertices; i++) {
			shortestPathAllVertices[i] = vector<int>(numberOfVertices, INT_MAX/3);
		}
	}
		...
		for(int k=0; k<numberOfVertices; k++){
			...
			for(int i=0; i<numberOfVertices; i++){
				for(int j=0; j<numberOfVertices; j++){
					if (shortestPathAllVertices[i][k] + shortestPathAllVertices[k][j] < shortestPathAllVertices[i][j])  
						shortestPathAllVertices[i][j] = shortestPathAllVertices[i][k] + shortestPathAllVertices[k][j];
				}
			}
		}	
	}
